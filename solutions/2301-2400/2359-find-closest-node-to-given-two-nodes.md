...
题目是功能图（每个点最多一条出边）。分别从 `node1` 和 `node2` 出发沿出边走，
记录每个节点第一次到达的距离（遇到 `-1` 或重复访问就停止，避免在环里死循环）。
然后遍历所有节点，筛出两边都可达的点，取 `max(dis1[i], dis2[i])` 最小的那个；
因为按下标从小到大扫描，只有严格更小时才更新，自然满足并列取最小下标。
- 时间复杂度: $O(n)$
- 空间复杂度: $O(n)$
