...
利用两个数组都非递增这一性质，直接做双指针扫描。

指针 `a` 枚举 `nums1[a]`，指针 `b` 始终尽量向右扩展，表示当前能和 `a` 配对的最远位置。  
如果 `nums1[a] <= nums2[b + 1]`，说明还能继续增大距离，就把 `b` 右移；否则当前 `b` 已经是 `a` 的最远可行位置，更新答案后把 `a` 右移，继续考察下一个下标。  
由于 `a` 和 `b` 都只会单调向右移动一次，所以整体是线性复杂度。
    ...
    def maxDistance(self, nums1: List[int], nums2: List[int]) -> int:
        ...
        while a < na and b < nb - 1:
            if nums1[a] <= nums2[b + 1]:
                ...
                # b 不动
        if a != na:
            ans = max(b - a, ans) # 由于 b 退出的越界需要额外计算一次
        return ans
```
