...
- 从 `start` 开始按距离一圈圈向外扩展，第一次遇到 `target` 时就是最小距离。
- 每一轮同时检查左边和右边，边界越界时就钳到数组范围内。
...
class Solution:
    def getMinDistance(self, nums: List[int], target: int, start: int) -> int:
        # 从 start 两边扩展搜索 target
        n = len(nums)
        ans = 0
        while True:
            l = start - ans if start - ans >= 0 else 0 
            r = start + ans if start + ans < n else n - 1
