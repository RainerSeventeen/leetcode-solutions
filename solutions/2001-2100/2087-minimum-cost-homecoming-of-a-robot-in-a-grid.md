...
- 机器人从起点走到终点时，行方向和列方向可以独立计算。
- 在行方向上，只需要累计 `start_row` 和 `home_row` 之间经过的所有行代价；列方向同理。
- 直接按区间求和即可，不需要模拟每一步移动，也不需要额外状态。
...
class Solution:
    def minCost(self, startPos: List[int], homePos: List[int], rowCosts: List[int], colCosts: List[int]) -> int:
        ...
        row_lo, row_hi = sorted((start_row, home_row))
        col_lo, col_hi = sorted((start_col, home_col))

        return sum(rowCosts[i] for i in range(row_lo + 1, row_hi + 1)) + sum(
            colCosts[i] for i in range(col_lo + 1, col_hi + 1)
        )
```
