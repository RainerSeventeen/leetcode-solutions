...
维护 `dp[i]` 表示当前图中 `0 -> i` 的最短距离，并保存所有新增有向边。
每次加边 `(s,e)` 后，若 `dp[s]+1` 能改进 `dp[e]`，就从 `e` 开始做队列松弛传播。
传播时同时尝试链式边 `i->i+1` 与新增边，直到没有点可继续变短。
- 时间复杂度: $O(q(n+q))$
- 空间复杂度: $O(n+q)$
...
class Solution:
    def shortestDistanceAfterQueries(self, n: int, queries: List[List[int]]) -> List[int]:
        ...
        ret = []
        dp = list(range(n))  # 初始链：0->i 的距离为 i
        for s, e in queries:
            edge[s].append(e)
            newd = dp[s] + 1
            if newd >= dp[e]:              # 没变短就别扩散
                ret.append(dp[n - 1])
                continue
            ...
            while q:
                ...
                # 常规链边 curr -> curr+1
                if curr < n - 1 and dp[curr + 1] > dp[curr] + 1:
                    dp[curr + 1] = dp[curr] + 1
                    q.append(curr + 1)

                # 新增边 curr -> nxt
                for nxt in edge[curr]:
                    if 0 <= nxt < n and dp[nxt] > dp[curr] + 1:
                        dp[nxt] = dp[curr] + 1
                        q.append(nxt)
