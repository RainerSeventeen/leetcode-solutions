...
直接枚举每个大小为 $k \times k$ 的子矩阵，把其中的 $k^2$ 个元素取出后排序，再扫描相邻元素的差值，取最小的非零差即可。

由于题目要求的是“不同值”之间的最小绝对差，所以排序后只需要比较相邻元素；若所有元素都相同，则答案为 `0`。

- 时间复杂度: $O((r-k+1)(c-k+1)\cdot k^2 \log k)$，其中 $r$ 是行数，$c$ 是列数
- 空间复杂度: $O(k^2)$
    ...
    def minAbsDiff(self, grid: List[List[int]], k: int) -> List[List[int]]:
        ...
        for i in range(n - k + 1):
            for j in range(m - k + 1):
                ...
                for nx in range(k):
                    for ny in range(k):
                        curr_mtx.append(grid[i + nx][j + ny])
                curr_mtx.sort()
                # print(curr_mtx)
                for idx in range(1, k ** 2):
                    delta = curr_mtx[idx] - curr_mtx[idx - 1]
                    if delta == 0:
                        continue
                    else:
                        min_abs = min(min_abs, delta)
                ans[i][j] = min_abs if min_abs != float('inf') else 0
        return ans
```
