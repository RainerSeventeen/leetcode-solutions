...
使用滑动窗口维护“最多包含两种水果”的区间，哈希表记录窗口内每种水果数量。
右指针扩张窗口；若种类超过 2，则移动左指针并同步减计数，直到恢复合法。
每轮更新窗口长度最大值即为答案。
- 时间复杂度: $O(n)$
- 空间复杂度: $O(1)$
...
public:
    int totalFruit(vector<int>& fruits) {
        int currentlen = 0;
        int i = 0, j = 0;
        // 如果满足后，记录值，和之前的值比较，计算最大值
        unordered_map <int, int> fmap;
        while (j < fruits.size()){
            // 左右双指针，right不断向右移动
            fmap[fruits[j]] += 1;   // 移动后要写入哈希表看看是否有数值
            j++;
            while (fmap.size() > 2 && i < j){
                // 在right移动后导致种类数量>=2时，内嵌一个left循环向右移动
                fmap[fruits[i]] -= 1;
                if (fmap[fruits[i]] == 0) 
                    fmap.erase(fruits[i]);
                i++;
