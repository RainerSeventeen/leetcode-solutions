...
把障碍物坐标放进哈希集合，之后机器人每次前进一步时都能在 $O(1)$ 时间判断是否撞到障碍物。命令逐个模拟即可：遇到转向命令就更新方向向量，遇到前进命令就按步推进，若下一格是障碍物就立刻停止当前命令。过程中持续维护当前位置到原点距离平方的最大值。

- 时间复杂度: $O(m + n)$，其中 $m$ 是 `obstacles` 数量，`n` 是所有前进步数之和，题目中每个前进命令至多走 9 步，因此也可视为与输入规模线性相关
- 空间复杂度: $O(m)$，用于存放障碍物哈希集合
...
class Solution:
    def robotSim(self, commands: List[int], obstacles: List[List[int]]) -> int:
        # 哈希放进所有的障碍物节点，每一步执行查询
        obs = {tuple(x) for x in obstacles} # 先转成 tuple
        dire = [0, 1] # 初始方向
        curr = [0, 0]
        ans = 0
        for cmd in commands:
            if cmd == -1:
                dire[0], dire[1] = dire[1], -dire[0]
            elif cmd == -2:
                dire[0], dire[1] = -dire[1], dire[0]
            else:
                for i in range(cmd):
                    nxt = [dire[0] + curr[0], dire[1] + curr[1]]
                    if tuple(nxt) not in obs:
                        curr = nxt
